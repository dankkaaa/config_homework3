...
; Описание: Вычисление произведения 4 * 7 = 28
...
; Инициализация
LOAD A, 4       ; A = 4 (первый множитель)
LOAD B, 7       ; B = 7 (второй множитель)
LOAD C, 0       ; C = 0 (результат)

; Умножение (повторяем 4 раза добавление 7)
mult_loop:
...
; Ожидаемый машинный код (hex):
; 01 00 04 01 01 07 01 02 00 03 02 01 04 00 01 0a 06 10
; Размер: 18 байт
...
; ============================================================================
; ТЕСТ 8: КОМПЛЕКСНАЯ ПРОГРАММА - ВЫЧИСЛЕНИЕ ФАКТОРИАЛА
; Описание: Вычисление факториала 5! = 120
; Входные данные: A=5
; Ожидаемый результат: B=120
; ============================================================================

TEST8_FACTORIAL:

; Инициализация
LOAD A, 5       ; A = 5 (исходное число)
LOAD B, 1       ; B = 1 (результат факториала)
LOAD C, 2       ; C = 2 (текущий множитель)

; Основной цикл: B = B * C, потом C++, пока C <= A
factorial_loop:
CMP C, A        ; Сравниваем C с A
JNZ skip_mul    ; Если C > A, выход из цикла

; Умножаем B на C
LOAD D, 0       ; D = 0 (промежуточный результат)
mult_c:
ADD D, B        ; D = D + B
SUB C, 1        ; C = C - 1
JNZ mult_c      ; Если C != 0, повторить умножение

; После этого D = B * (исходное C)
; Но это упрощённая версия, полная требует доп. логики

HALT

; Примечание: Полная реализация факториала более сложная
